...
# Prevent OpenCV's bundled Qt plugins from conflicting with PySide6
...
class RealtimeWindow(QMainWindow):
    def __init__(self, defaults: argparse.Namespace) -> None:
        ...
        # Auto-scale to fit screen (use 92% of available area)
    ...
    def _build_ui(self) -> None:
        ...
        metric_form.addRow("FPS", self.lbl_fps)
    ...
    def _update_metrics_ui(self) -> None:
        ...
        self.lbl_fps.setText(f"{self.fps_val:0.1f}")
    ...
    def _tick(self) -> None:
        ...
        if now - self.fps_last_time >= 1.0:
            self.fps_val = self.fps_count / (now - self.fps_last_time)
            self.fps_count = 0
        ...
        cv2.putText(frame, f"FPS={self.fps_val:0.1f}", (frame.shape[1] - 130, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.56, (200, 200, 200), 2)
