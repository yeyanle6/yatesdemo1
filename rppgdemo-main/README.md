...
```bash
cd /Users/liangwenwang/Downloads/Code/Demo2/rppgdemo-main
...
- Desktop 锁定版本：`requirements-desktop-lock.txt`
...
脚本：`/Users/liangwenwang/Downloads/Code/Demo2/rppgdemo-main/evaluate_dataset.py`

一次跑两种 ROI + timestamp 对齐 + HR/LF 指标（推荐）：

```bash
python evaluate_dataset.py \
  --data-dir /Users/liangwenwang/Downloads/Code/Demo2 \
  --out-dir /Users/liangwenwang/Downloads/Code/Demo2/rppgdemo-main/results \
  --roi-mode both \
  --align-mode timestamp \
  --metrics hr,lf \
  --split-file /Users/liangwenwang/Downloads/Code/Demo2/rppgdemo-main/splits/fixed_holdout_6.txt
```
...
脚本：`/Users/liangwenwang/Downloads/Code/Demo2/rppgdemo-main/auto_optimize.py`

能力：
- 三阶段搜索：参数粗搜 -> 变体进化 -> 局部精修
- train/test 固定切分门禁（防止只在训练集变好）
- 目标函数：`HR + LF` 平衡（`hr_lf_balanced`）
- 变体签名、自动回滚、详细审计产物
...
python auto_optimize.py \
  --data-dir /Users/liangwenwang/Downloads/Code/Demo2 \
  --out-dir /Users/liangwenwang/Downloads/Code/Demo2/rppgdemo-main/results/auto_opt \
  --roi-mode mediapipe \
  --search-budget 60 \
  --allow-code-mutation \
  --objective-profile hr_lf_balanced \
  --metrics hr,lf \
  --split-file /Users/liangwenwang/Downloads/Code/Demo2/rppgdemo-main/splits/fixed_holdout_6.txt
```
...
python auto_optimize.py \
  ...
  --holdout-list 001/3-3,002/3-4
```

每次运行会在 `results/auto_opt/<timestamp>/` 生成：
- `iter_XXX_train_detail.csv`
- `iter_XXX_test_detail.csv`
